 ...
 */
export const fetchUserProfiles = async (apiBaseUrl, userId) => {
  if (!userId) {
  ...
  const url = `${apiBaseUrl}/user/${userId}/profiles`;
  ...
  if (!response.ok) {
    ...
    throw new Error(errorData.message || `Failed to fetch user profiles. Status: ${response.status}`);
  }
